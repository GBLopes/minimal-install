...
echo "${GREEN}Atualizando permissões da pasta do usuário ${USER}...${NC}" && \
chmod 755 /home/$USER && \

# Add sbin
echo "${GREEN}Atualizando .bashrc do usuário ${USER} com sbin...${NC}" && \
./utils/add_sbin.sh && \

# Add nvim
echo "${GREEN}Atualizando .bashrc do usuário ${USER} com nvim...${NC}" && \
./utils/add_nvim.sh && \

# Add zellij
echo "${GREEN}Atualizando .bashrc do usuário ${USER} com zellij...${NC}" && \
./utils/add_zellij.sh && \

# Habilitando alias em .bashrc
echo "${GREEN}Habilitando alias em .bashrc do usuário ${USER}...${NC}" && \
...
echo "${GREEN}Adicionando lançadores na dock do usuário ${USER}...${NC}" && \
...
# config picom
echo "${GREEN}Configurando picom do usuário ${USER}...${NC}" && \
...
# config zsh
echo "${GREEN}Configurando zsh do usuário ${USER}...${NC}" && \
./utils/config_zsh.sh && \

# config alacritty
echo "${GREEN}Configurando alacritty do usuário ${USER}...${NC}" && \
./utils/config_alacritty.sh && \

# config gedit
echo "${GREEN}Configurando gedit do usuário ${USER}...${NC}" && \
